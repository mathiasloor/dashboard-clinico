...
def platform_sales_bubble_chart(df):
    ...
    fig.update_layout(xaxis_title="Puntuación Media de Críticos", yaxis_title="Puntuación Media de Usuarios", legend_title="Plataformas")
    return fig
...
# Consola de resumen con estadísticas clave
def summary_console(df):
    total_games = df['Name'].shape[0]
    avg_critic_score = df['Critic_Score'].mean()
    avg_user_score = df['User_Score'].mean()
    total_sales = df['Global_Sales'].sum()
    return html.Div([html.H5(f"Total de Juegos: {total_games}"),
...
sidebar = html.Div(
    [
        ...
        html.Hr(),
        html.P("Análisis de Ventas y Puntaciones de Videojuegos", className="lead"),
        dbc.Nav(
            [
                dbc.NavLink("Ventas por Género", href="/sales", active="exact", id="link-sales"),
                dbc.NavLink("Puntuaciones y Ventas", href="/platforms", active="exact", id="link-platforms"),
                dbc.NavLink("Palabras mas comunes", href="/wordcloud", active="exact", id="link-wordcloud"),
                dbc.NavLink("Ventas Regionales y Puntuaciones", href="/heatmap", active="exact", id="link-heatmap"),
                dbc.NavLink("Puntuaciones en el Tiempo", href="/line-chart-scores", active="exact", id="link-linechart"),
                dbc.NavLink("Ventas Regionales (Top 10 Plataformas)", href="/regional-sales", active="exact", id="link-regional-sales"), 
                dbc.NavLink("Ventas Globales (Top 5 Plataformas)", href="/sunburst", active="exact", id="link-sunburst")  # Enlace al Sunburst
            ],
            vertical=True,
            pills=True,
        ),
        ...
        html.Div(
            dbc.Button("Seleccionar Todo", id="select_all_genres", n_clicks=0, 
                       color="primary", className="me-1", style={'margin-top': '10px', 'width': '100%'})
        ),
...
# Layout del dashboard con barra lateral y descripción fija
app.layout = html.Div([
    dcc.Location(id="url"),
    sidebar,
    html.Div([html.H3("Descripción del Dataset de Ventas de Videojuegos"),
              html.P("Este dataset contiene información sobre más de 16,000 videojuegos, incluyendo "
                     "nombre, plataforma, género, ventas en distintas regiones y calificaciones de críticos y usuarios."),
              html.P("A continuación, se presentan varias visualizaciones interactivas para analizar las ventas, "
                     "calificaciones y nombres de los videojuegos más populares.")], style={"padding-left": "20%"}),
    content
])
...
# Función para controlar el contenido dinámico según la URL
@app.callback(
    Output("page-content", "children"),
    [Input("url", "pathname"), Input("genre_filter", "value"), Input("platform_filter", "value"),
     Input("year_filter", "value"), Input("critic_score_filter", "value"), Input("user_score_filter", "value"),
...
def render_page_content(pathname, genre_filter, platform_filter, year_filter, critic_score_filter, user_score_filter, rating_filter):
    ...
    if pathname == "/sales":
        return html.Div([html.H3("Ventas Globales por Género"), dcc.Graph(figure=genre_sales_bar_chart(df_filtered)),
                         html.Hr(), html.Div(summary_console(df_filtered))])
